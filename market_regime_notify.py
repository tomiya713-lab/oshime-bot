...
- **最大3本**まで。1本あたり「見出し（ソース）」＋『要約1行』＋『関係1行』＋URL（合計2行＋URL1行まで）
- 候補一覧に無いURLは絶対に書かない（該当が無ければ「該当ニュースなし」と書く）
- 最後にRSS候補（参照）も5本つける（label + URL）
- ハルシネーションは一切禁止
...
(2) 2通目：原因ニュース（最大3本）
- 候補から「原因になった可能性が高い順」に最大3本
- 1本あたり『要約1行』＋『理由1行』＋URL（合計2行＋URL1行まで）
- 候補一覧に無いURLは絶対に書かない（該当が無ければ「該当ニュースなし」と書く）
- 最後にRSS候補（参照）を5本（label + URL）
- ハルシネーションは一切禁止
