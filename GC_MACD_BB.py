...
def latest_gc_macd_bb_from_raw(raw_df: pd.DataFrame, ticker: str):
    ...
    try:
        ...
        # 最新週（進行中の週を含む）の終値が上限以上ならブレイク週扱い
        wbb_upper_touch = False
        if len(w_upper.dropna()) > 0:
...
def notify(df: pd.DataFrame, raw_df: pd.DataFrame):
    ...
    if df is None or df.empty:
        msg = (
            f"【GC_short×MACD×週足BB上限ブレイク】{now_jst():%m/%d %H:%M}\n"
            f"条件: GC_short(5>25クロス) & MACD>0 & MACD>Signal & 週足BB(20週+2σ)上限ブレイク週\n"
            f"該当銘柄はありませんでした。"
        )
        send_long_text(msg)
        return
    ...
    for _, r in df.iterrows():
        ...
        sma25 = fp(r["SMA25"], 2)
        macd = fp(r["MACD"], 3)
        sig = fp(r["MACD_signal"], 3)
        line = f"{t:<8} {name:<8}  SMA5:{sma5:>8}  SMA25:{sma25:>8}  MACD:{macd:>7}  Sig:{sig:>7}"
        lines.append(line)
