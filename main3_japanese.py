...
def signal_lines(df: pd.DataFrame, level: str, title: str) -> List[str]:
    """銘柄一覧の指標名をすべて日本語で整形する。"""
    ...
    for _, row in subset.iterrows():
        ...
        lines.append(
            f"{ticker:<8} {name:<8} "
            f"戻り余地:{app._fp(row.get('Return_%'), 1):>5}% "
            f"高値下落率:{app._fp(row.get('Drop_From_Peak_%'), 1):>5}% "
            f"押し目戻り率:{app._fp(row.get('Delta_from_Pull_%'), 1):>5}% "
            f"判定期間:{int(row.get('Window') or 0):>2}日 "
...
def notify_japanese(df: pd.DataFrame, raw_df: pd.DataFrame):
    """黄色・緑の通知本文と画像説明をすべて日本語化する。"""
    condition_text = (
        f"共通条件: 高値からの下落率≤{core.DROP_MAX:.0f}%・"
        f"戻り余地≥{core.EXPECTED_RISE_MIN:.0f}%・"
        "25日移動平均>75日移動平均・75日移動平均上向き\n"
        f"🟡 押し目接近: 押し目安値+{app.APPROACH_BAND_PCT:g}%以内"
        "（反発未確認も対象）\n"
        f"🟢 反発確認: 押し目安値+{app.CONFIRMED_BAND_PCT:g}%以内・"
        "押し目日は前営業日以前・最新終値が前日終値超え\n"
        "※週足2週移動平均下向きは除外せず警告表示"
    )
    ...
    for _, row in ordered.iterrows():
        ...
        description = (
            ...
            f"判定期間: {int(row.get('Window') or 0)}日  "
            f"相対力指数(14): {rsi_text}{weekly_note}\n"
